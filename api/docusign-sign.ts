 ...
 *
 * Creates a DocuSign envelope for a timesheet, sends it for signing,
 * and stores the envelope ID + signing URL on the timesheet_entries row.
 *
 * Body: { orgId, personId, year, month, userId }
 *
 * Env vars required:
 *   DOCUSIGN_INTEGRATION_KEY   — DocuSign app integration key (client ID)
 *   DOCUSIGN_USER_ID           — DocuSign API user ID (impersonation)
 *   DOCUSIGN_ACCOUNT_ID        — DocuSign account ID
 *   DOCUSIGN_RSA_PRIVATE_KEY   — RSA private key (PEM, newlines as \n)
 *   DOCUSIGN_BASE_URL          — e.g. https://demo.docusign.net/restapi (demo) or https://na4.docusign.net/restapi (prod)
 *   DOCUSIGN_OAUTH_BASE_URL    — e.g. https://account-d.docusign.com (demo) or https://account.docusign.com (prod)
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const { data: org } = await supabase
      ...
      .eq('id', orgId)
      .single()
    const orgName = (org as any)?.name || 'Organisation'
    ...
    if ((envelope as any).status !== 'Submitted') {
    ...
    const workingDays = (envelope as any).working_days || 0

    // 5. Build HTML document
    const html = buildTimesheetHtml({
      personName: (person as any).full_name,
      orgName,
    ...
    const envelopePayload = {
      emailSubject: `Timesheet for signing: ${MONTHS[month - 1]} ${year} — ${(person as any).full_name}`,
      emailBlurb: `Please review and sign your timesheet for ${MONTHS[month - 1]} ${year}.`,
      documents: [{
        documentId: '1',
        name: `Timesheet_${(person as any).full_name.replace(/\s+/g, '_')}_${MONTHS[month - 1]}_${year}.html`,
    ...
    if (!createRes.ok) {
      ...
      console.error('[docusign-sign] Envelope creation failed:', errText)
    ...
    const viewPayload = {
      ...
      email: (person as any).email,
      userName: (person as any).full_name,
    ...
    if (!viewRes.ok) {
      ...
      console.error('[docusign-sign] Recipient view failed:', errText)
    ...
    if (userId) {
      ...
      if ((personUser as any)?.user_id) {
        await (supabase as any).from('notifications').insert({
          user_id: (personUser as any).user_id,
          org_id: orgId,
          type: 'approval',
