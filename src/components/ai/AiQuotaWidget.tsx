    ...
    return (
      <div className={cn('flex items-center gap-2 text-xs', className)}>
        ...
        <span className="text-muted-foreground whitespace-nowrap">
          {formatTokens(tokensRemaining)} {t('ai.tokensLeft')}
        </span>
      </div>
    )
  }
    ...
    <div className={cn('rounded-lg border p-4 space-y-3', isExhausted ? 'border-red-200 dark:border-red-800 bg-red-50/50 dark:bg-red-950/20' : 'bg-muted/30', className)}>
      ...
      <div className="space-y-1">
        <div className="flex items-center justify-between text-xs">
          ...
          <span className={cn('font-medium', isExhausted ? 'text-red-600 dark:text-red-400' : '')}>
            {formatTokens(tokensUsed)} / {formatTokens(limits.monthly_tokens)}
          </span>
        </div>
      ...
      <div className="space-y-1">
        <div className="flex items-center justify-between text-xs">
          ...
          <span className={cn('font-medium', isExhausted ? 'text-red-600 dark:text-red-400' : '')}>
            {requestsUsed} / {limits.monthly_requests}
          </span>
        </div>
        <div className="h-2 w-full rounded-full bg-muted overflow-hidden">
          <div
      ...
      {isExhausted ? (
        ...
        <p className="text-xs text-amber-600 dark:text-amber-400 flex items-center gap-1.5">
          ...
          {t('ai.quotaWarning', { remaining: formatTokens(tokensRemaining) })}
        </p>
      ) : (
        <p className="text-xs text-muted-foreground flex items-center gap-1.5">
          <TrendingUp className="h-3 w-3 shrink-0" />
          {t('ai.quotaOk', { remaining: formatTokens(tokensRemaining), requests: requestsRemaining })}
        </p>
      )}
    </div>
  )
